...
#if __SYS_COMM_LOG_ENABLE__
#define COMM_PrintLog(fmt,arg...)          do{\
	if(g_dev_prm.nvram.debug_log_st.comm_log)\
	PrintLog("[COMM]:"fmt,##arg);\
}while(0)
#define COMM_PrintHex(pdat,dat_len)          do{\
	if(g_dev_prm.nvram.debug_log_st.comm_log)\
				{DEBUG_Print("[COMM]:HEX:");DEBUG_PrintHex(pdat,dat_len);}\
		}while(0)
		
#define COMM_PrintChar(pdat,dat_len)          do{\
	if(g_dev_prm.nvram.debug_log_st.comm_log)\
				{DEBUG_Print("[COMM]:CHAR:");DEBUG_PrintChar(pdat,dat_len);}\
		}while(0)
#else
#define COMM_PrintLog(fmt,arg...)
#define COMM_PrintHex(pdat,dat_len)
#define COMM_PrintChar(pdat,dat_len)
#endif
